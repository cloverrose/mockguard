...
fmt:
	...
	gci write . --skip-generated --skip-vendor --custom-order -s standard -s default -s 'prefix(github.com/gostaticanalysis)' -s 'prefix(github.com/cloverrose)' -s 'prefix(github.com/cloverrose/mockguard)'
...
# build/mockguard creates the mockguard binary.
.PHONY: build/mockguard
build/mockguard:
	@CGO_ENABLED=0 go build -o bin/mockguard -v ./cmd/mockguard
